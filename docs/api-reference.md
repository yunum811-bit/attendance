...
> ⚠️ API Key ตั้งค่าผ่าน environment variable `API_KEY` บน server
> ⚠️ ติดต่อผู้ดูแลระบบเพื่อขอ API Key

**ตัวอย่างการเรียก (cURL):**
```bash
curl -H "X-API-Key: <your-api-key>" \
  "https://checkin.serialfactoring.co.th/api/external/attendance?start_date=2026-06-01&end_date=2026-06-30"
```

**ตัวอย่างการเรียก (JavaScript):**
```javascript
...
| มาตรการ | รายละเอียด |
|---|---|
| Key ยาว 64 ตัวอักษร (256-bit) | ป้องกันการเดา/brute force |
| Timing-safe comparison | ป้องกัน timing attack |
| Rate limiting | 100 req/นาที ต่อ IP |
| Environment variable | Key ไม่อยู่ในโค้ด ไม่ commit ลง git |
...
```bash
curl -H "X-API-Key: <your-api-key>" \
  "https://checkin.serialfactoring.co.th/api/external/attendance?department_id=2&start_date=2026-05-01&end_date=2026-05-31"
```
...
```bash
curl -H "X-API-Key: <your-api-key>" \
  "https://checkin.serialfactoring.co.th/api/external/leave?department_id=2&status=approved&start_date=2026-01-01&end_date=2026-12-31"
```
...
```javascript
// เก็บ API Key ใน environment variable — ห้าม hardcode ในโค้ด
...
// ตรวจใครมาสาย (หลัง 08:30)
const lateEmployees = attendance.filter(a => a.check_in > '08:30:00');
console.log(`วันนี้มาสาย ${lateEmployees.length} คน`);
```
...
*API Reference ปรับปรุงล่าสุด: 19 มิถุนายน 2026*
